...
static int pixusip8600_setalert(bscc2sts_tbl *p_tbl, int color_num);

ST_BSCC2STS pixusip8600_cir2fill[]={
  {"100", "100"},
  {"070", "070"},
  {"040", "040"},
  {"010", "010"},
...
int pixusip8600_setink(ST_STORESET *p_s, ST_BSCC2STS *p_ct, bscc2sts_tbl *p_tbl, ST_BSCCBUF *p_bsccbuf)
{
  ...
  while( (p_tok=strtok(NULL, ",")) != NULL ){
	ret=commonstssetprocess(p_tok,p_tbl->ink[i].fill, &pixusip8600_cir2fill[0]);
	if(ret == BADITEM ){
	  memset(&p_tbl->ink[0], NOITEM, p_s->size);
	  return(OK);
	}
  ...
  ret=pixusip8600_setalert(p_tbl, color_num);
  return(OK);
}


int pixusip8600_setalert(bscc2sts_tbl *p_tbl, int color_num)
{
  int i,ret;
