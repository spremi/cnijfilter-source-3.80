...
/* for ip7500 conversion table*/

/*
 * The present busy detailed status.
 */
ST_BSCC2STS ip7500_dbs2busy[]={
  {"NO"," "},
  {"UK","?"},
//{"WU","S"},
//{"SL","E"},
 ...
 */
ST_BSCC2STS ip7500_chd2type[]={
  {"NO"," "},
  {"LS","R"},
  {ENDTAG,ENDTAG}
};

ST_BSCC2STS ip7500_prname2exchange[]={
  {"IP7500","F"},
  {ENDTAG,ENDTAG}
};
 ...
 */
ST_BSCC2STS ip7500_doc2operate[]={
  {"NO"," "},
  {"UK","?"},
  {"1000","V"},
  {"1001","R"},
 ...
 */
ST_BSCC2STS ip7500_dsc2service[]={
  {"NO"," "},
  {"UK","?"},
  {"5100","5100"},
  {"5110","5110"},
...
/*
 * The function table for ip7500.
 */
static const FUNCOFMODELSETPROCESS
 ip7500setstsfunctable[] = {
  NULL,
  NULL,
  NULL,
  NULL,
 ...
 */
ST_BSCC2STS *p_ip7500chgtbl[] = {
  NULL,
  ip7500_dbs2busy,
  ip7500_djs2job,
  ip7500_cil2inkchk,
  ip7500_chd2type,
  ip7500_prname2exchange,
  ip7500_lvr2posit,
  ip7500_dws2warn,
  ip7500_doc2operate,
  ip7500_dsc2service,
...
int processforip7500(ST_STORESET *p_s, bscc2sts_tbl *p_tbl, ST_BSCCBUF *p_bsccbuf)
{
  ...
  for(i=0; i<MAXFUNCNUM; i++ ){
    if( ip7500setstsfunctable[i] == NULL){
      ret = selectcommonfunc(p_s+i, p_ip7500chgtbl[i], p_tbl, i);
      if(ret != OK){
		break;
      }
    } else {
      ret = (*ip7500setstsfunctable[i])(p_s+i, p_ip7500chgtbl[i], p_tbl, p_bsccbuf);
      if(ret != OK){
		break;
      }
    }
