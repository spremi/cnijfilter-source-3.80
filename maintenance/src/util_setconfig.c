...
SETCONFIGTABLE setConfigTableAutoPowerOff[] = {
	{ "autopower_disable", BJL_STR_DISABLE, MSI_STR_OFF, 0 },
	{ "autopower_5min", BJL_STR_005_MIN, MSI_STR_05_MIN, 1 },
	{ "autopower_10min", BJL_STR_010_MIN, MSI_STR_10_MIN, 2 },
	{ "autopower_15min", BJL_STR_015_MIN, MSI_STR_15_MIN, 3 },
	{ "autopower_30min", BJL_STR_030_MIN, MSI_STR_30_MIN, 4 },
	{ "autopower_60min", BJL_STR_060_MIN, MSI_STR_60_MIN, 5 },
	{ NULL, NULL, NULL, -1 },
};
...
SETCONFIGTABLE setConfigTableAutoPowerOff03[] = {
	{ "autopower_5min", BJL_STR_BT005_MIN, MSI_STR_05_MIN, 0 },
	{ "autopower_10min", BJL_STR_BT010_MIN, MSI_STR_10_MIN, 1 },
	{ "autopower_15min", BJL_STR_BT015_MIN, MSI_STR_15_MIN, 2 },
	{ "autopower_30min", BJL_STR_BT030_MIN, MSI_STR_30_MIN, 3 },
	{ NULL, NULL, NULL, -1 },
};
...
/* for cnnetlib */
static const gchar* CNNET_LIB_NAME = "libcnnet.so";

static const gchar* CNNL_INIT = "CNNL_Init";
static const gchar* CNNL_GETIPADRESSEX = "CNNL_GetIPAddressEx";
static const gchar* CNNL_OPENEX = "CNNL_OpenEx";
static const gchar* CNNL_CLOSE = "CNNL_Close";
static const gchar* CNNL_GETCOMMANDTYPE = "CNNL_GetCommandType";
static const gchar* CNNL_GETDEVICEID = "CNNL_GetDeviceID";
static const gchar* CNNL_TERMINATE = "CNNL_Terminate";

static int (*cnnl_init)(CNNLHANDLE *h);
static int (*cnnl_getipaddressex)(CNNLHANDLE h, const char *cachefile, const char *macaddr, char *ipaddr, const unsigned long bufsz, int mode, const int retry, const unsigned long timeout);
static int (*cnnl_openex)(CNNLHANDLE h, const char *host, int command_type, const int retry, const unsigned long timeout);
static int (*cnnl_close)(CNNLHANDLE h);
static int (*cnnl_getcommandtype)(CNNLHANDLE h, int *commandtype, const int retry, const unsigned long timeout);
static int (*cnnl_getdeviceid)(CNNLHANDLE h, void *buf, unsigned long *readsz, const unsigned long bufsz, const int retry, const unsigned long timeout);
static int (*cnnl_terminate)(CNNLHANDLE *h);
...
static gboolean IsAutoPower( int type )
{
	...
	switch ( type ) {
		...
		case UI_AUTOPOWER_TYPE005:	/* Ver.3.80 */
			ret = TRUE;
			break;
		default:
			break;
...
static void SetUISetConfigType001_AutoPower( int uiType, GtkWidget *dialog, int isDefault  )
{
	...
	else if ( (uiType == UI_AUTOPOWER_TYPE002) || (uiType == UI_AUTOPOWER_TYPE005) ){
		if ( isDefault ){
			SetComboBoxItemsFromSetConfigTable(combo1, setConfigTableAutoPowerOn, onSize, 0 );
...
static int CreateMSIListViaNet( GList **gList, char *uriBuf, int uriBufSize, int retry, int timeout )
{
	...
	*gList = NULL;
	...
	/* load cnnet lib */
	if ( (handle = dlopen( CNNET_LIB_NAME, RTLD_NOW )) == NULL ) goto onErr1;

	if ( (cnnl_init = dlsym( handle, CNNL_INIT )) == NULL ) goto onErr2;
	if ( (cnnl_getipaddressex = dlsym( handle, CNNL_GETIPADRESSEX )) == NULL ) goto onErr2;
	if ( (cnnl_openex = dlsym( handle, CNNL_OPENEX )) == NULL ) goto onErr2;
	if ( (cnnl_close = dlsym( handle, CNNL_CLOSE )) == NULL ) goto onErr2;
	if ( (cnnl_getcommandtype = dlsym( handle, CNNL_GETCOMMANDTYPE )) == NULL ) goto onErr2;
	if ( (cnnl_getdeviceid = dlsym( handle, CNNL_GETDEVICEID )) == NULL ) goto onErr2;
	if ( (cnnl_terminate = dlsym( handle, CNNL_TERMINATE )) == NULL ) goto onErr2;

	/* init */
	//if ( CNNL_Init(&gHandle) != CNNL_RET_SUCCESS ) goto onErr2;
	if ( cnnl_init(&gHandle) != CNNL_RET_SUCCESS ) goto onErr2;
	...
	if ( status != CNNL_RET_SUCCESS ) goto onErr3;
	...
	if( commandType != CNNL_COMMAND_IVEC ) goto onErr4;
	...
	if ( IsCustomSettings( gUiType )  ){
		...
		if ( gUiType == UI_CUSTOMSETTINGS_TYPE003 ){
			...
			if ( (index = SearchSetConfigTableIndex( setConfigTableWFeed, GetMSIStr( gList, MSI_TYPE_WFEED) )) != -1 ) {
				...
				//fprintf( stderr, "gUiType : %d\n", gUiType );
		/* init */
		InitCreate_SetConfigType001_AutoPower( gUiType );

		if ( (gUiType == UI_AUTOPOWER_TYPE001) || (gUiType == UI_AUTOPOWER_TYPE002)  || (gUiType == UI_AUTOPOWER_TYPE005)){
			/* Auto Power ON */
		...
		/* Auto Power OFF */
		if ( (gUiType == UI_AUTOPOWER_TYPE002) || (gUiType == UI_AUTOPOWER_TYPE003) || (gUiType == UI_AUTOPOWER_TYPE005)){
			...
			//fprintf( stderr, "Auto Power OFF<2>\n" );
				gAutoPowerOffIndex = index;
				//fprintf( stderr, "gAutoPowerOffIndex : %d\n", gAutoPowerOffIndex );
			}
		}
		/* Auto Power OFF */
...
int GetMSIPaperSourceSettings01( int *paperSource )
{
	...
	*paperSource = CND_SUPPLY_CASSETTE;
	...
	else if ( !strcmp( strPtr, MSI_STR_2FSF )){ /* Ver.3.80 */
		*paperSource = CND_SUPPLY_CASSETTE_AUTO;
	}
	else goto onErr;
...
static short RetFunctionStatusWithConnectSwitch( int uiType, short okStatus, short ngStatus )
{
	...
	//fprintf( stderr, "RetFunctionStatusWithConnectSwitch <1>\n" );
	if ( ParseDeviceURI( &type, uriBuf, URI_BUF_SIZE ) == -1 ) goto onErr;
...
short ExecSetConfigType000_017( char *dialogName ){ return RetFunctionStatusWithConnectSwitch( UI_AUTOPOWER_TYPE005, US_F_AUTO_POWER, US_F_AUTO_POWER_CONNECTIONOFF); } /* Ver.3.80 */
short ExecSetConfigType000_018( char *dialogName ){ return RetFunctionStatusWithConnectSwitch( UI_AUTOPOWER_TYPE002, US_F_AUTO_POWER, US_F_AUTO_POWER_CONNECTIONOFF); } /* Ver.3.80 */
