...
PRIVATE const gchar	*gSTSMessageTable[] = {
	...
	N_("The remaining level of the following ink cannot be correctly detected.\n"),
	...
	N_("Printer detected ink out condition of the following ink.\n"),
	...
	N_("Fit paper into the cassette (lower).\n"),
	...
	N_("Check that there are no objects in front of the printer and that the cassette (upper) is fully inserted, and then open the paper output tray.\n"),
	/* ID_STS_MESSAGE_OC_PAPER_JAM_BACK12_1 */
	N_("Detach the rear cover.\nOpen the paper output tray if it is closed.\n"),
	/* ID_STS_MESSAGE_OC_PAPER_JAM_BACK12_1_2 */
	N_("Detach the rear cover, and then detach the transport unit cover inside.\nOpen the paper output tray if it is closed.\n"),
	/* ID_STS_MESSAGE_OC_PAPER_JAM_BACK12_2 */
	N_("Remove the jammed paper from inside the transport unit.\nIf the paper cannot be removed, refer to the user's guide to remove it.\n"),
	/* ID_STS_MESSAGE_OC_PAPER_JAM2_3rd_TAP */
	N_("Tap [OK] on the printer. Printing resumes from the next paper.\n"),
	/* ID_STS_MESSAGE_BUSY_HEADCHANGING */
	N_("The print head holder has moved to the head replacement position.\n\n"),
	/* ID_STS_MESSAGE_OC_COVER_OPEN3 */
	N_("The printer's cover is open.\nIf printing is in progress, the ink tank cannot be replaced. Close the cover, wait for printing to finish, then open the cover again and replace the ink tank.\n\n"),
	/* ID_STS_MESSAGE_SC_SERVICE_ERROR_CARRIAGEPOS_DISPO */
	N_("A printer error has occurred.\nTurn the printer off. Then check that there are no foreign objects in the printer and that the ink cartridges are installed properly.\nAfter that, turn the printer on again.\n\nIf this doesn't clear the error, the printer needs repair. See the user's guide and contact the service center.\n"),
	/* ID_STS_MESSAGE_OC_INKOUT06_TANK_TAP */
	N_("\nReplacing the ink tank is recommended.\nIf printing is in progress and you want to continue printing, tap [OK] on the printer. Then printing can continue. Replacing the ink tank is recommended after the printing. The printer may be damaged if printing is continued under the ink out condition.\n"),
	/* ID_STS_MESSAGE_OC_USEDTANK_ALMOST_TAP */
	N_("The ink absorber is almost full.\nTap [OK] on the printer to continue printing.\nContact the service center.\n"),
	/* ID_STS_MESSAGE_OC_USEDTANK_ALMOSTJP_TAP */
	N_("The ink absorber is almost full.\nTap [OK] on the printer to continue printing.\nThe printer will soon require servicing to replace the ink absorber.\nContact the service center for replacement.\nThe ink absorber can only be replaced by a qualified technician.\n"),
	/* ID_STS_MESSAGE_OC_DUPLEX_STOPPAGE2_TAP */
	N_("Duplex printing cannot be continued because the size of the paper loaded in the printer is not supported by the duplex printing function or a paper size that does not allow duplex printing was specified. Load paper of the correct size in the printer, check the paper size setting, then tap [OK] on the printer.\n"),
	/* ID_STS_MESSAGE_OC_HEADALIGNMENT_TAP */
	N_("An error occurred during automatic print head alignment.\n\nThe print head nozzles may be clogged. Execute cleaning on the Maintenance tab of the printer driver, and reexecute print head alignment.\n\nIf an error still occurs, refer to the user's guide to check the error or carry out manual alignment.\n\nTap [OK] on the printer to clear the error.\n"),
			...
			if (pPrinterStatus != NULL) {
				...
				for (i = 0; i < pPrinterStatus->messageNum; i++) {
					...
					for(x=0; x<8; x++){
						...
						if( ( (*pMessageID == ID_STS_MESSAGE_INK_GLAY0)    && (pPrinterStatus->status_ifnumber[x].colorstr == 'H') )
						||  ( (*pMessageID == ID_STS_MESSAGE_INK_BLACK1)   && (pPrinterStatus->status_ifnumber[x].colorstr == 'l') )
						||  ( (*pMessageID == ID_STS_MESSAGE_INK_BLACK0)   && (pPrinterStatus->status_ifnumber[x].colorstr == 'L') )
						||  ( (*pMessageID == ID_STS_MESSAGE_INK_CYAN0)    && (pPrinterStatus->status_ifnumber[x].colorstr == 'C') )
						||  ( (*pMessageID == ID_STS_MESSAGE_INK_MAGENTA0) && (pPrinterStatus->status_ifnumber[x].colorstr == 'M') )
						||  ( (*pMessageID == ID_STS_MESSAGE_INK_YELLOW0)  && (pPrinterStatus->status_ifnumber[x].colorstr == 'Y') ) ){
							gchar * msgPtr = dgettext(PACKAGE, gSTSMessageTable[*pMessageID]);
							gchar * message1
								= (gchar*)g_malloc(strlen(msgPtr) + 32);
							gchar * message2
								= (gchar*)g_malloc(32);
							gchar inkname[32];
							int xxx = 0;
							...
							sprintf( message1, "%s", msgPtr );
							for(xxx=0;xxx<sizeof(inkname);xxx++){
								inkname[xxx] = pPrinterStatus->status_ifnumber[x].ifnumberstr[xxx];
								if( (pPrinterStatus->status_ifnumber[x].ifnumberstr[xxx] == ' ')
								&&  (pPrinterStatus->status_ifnumber[x].ifnumberstr[xxx+1] == ' ') )
								{
									inkname[xxx+1] = '\0';
									break;
								}
							}
							//printf("pPrinterStatus->status_ifnumber[x].ifnumberstr[xxx] = %s\n",pPrinterStatus->status_ifnumber[x].ifnumberstr);
							//sprintf( message2, "%s", pPrinterStatus->status_ifnumber[x].ifnumberstr );
							sprintf( message2, "%s",inkname );
					...
					if (strlen(pPrinterStatus->serviceCallID) == 4) {
						...
						gchar * message4
							= (gchar*)g_malloc(strlen(msgPtr) + strlen(pPrinterStatus->serviceCallID) + 1);
						//printf("gSTSMessageTable[%d] = %s\n",*pMessageID, gSTSMessageTable[*pMessageID]);
						sprintf( message3, "%s", msgPtr );
						sprintf( message4, message3, pPrinterStatus->serviceCallID );

						if( inkifnumberflag == 0 )
							insertStringToText(STR_MAIN_TEXT_STATUS, message4);
